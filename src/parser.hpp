                        ...
                        int type) : Node(op_type, left, right)
  {
    ...
    if (left != NULL)
    {
      if (left->op_type == LOGICAL_RELATIONAL)
      {
        ...
        (((LogicalRelationalNode *)left)->lr).vect.at(0).propagateList.push_back(make_pair(make_pair(this, -0.1), LEFT));
      }
    }
    if (right != NULL)
    {
      if (right->op_type == LOGICAL_RELATIONAL)
      {
        ...
        (((LogicalRelationalNode *)right)->lr).vect.at(0).propagateList.push_back(make_pair(make_pair(this, -0.1), RIGHT));
      }
    }
  }
  LogicalRelationalNode(opType op_type,
    ...
    if (left != NULL)
    {
      if (left->op_type == LOGICAL_RELATIONAL)
      {
        for (int i = 0; i < (((LogicalRelationalNode *)left)->lr).vect.size(); i++)
        {
          ...
          (((LogicalRelationalNode *)left)->lr).vect.at(i).propagateList.push_back(make_pair(make_pair(this, -0.1), LEFT));
        }
      }
    }
    if (right != NULL)
    {
      if (right->op_type == LOGICAL_RELATIONAL)
      {
        for (int i = 0; i < (((LogicalRelationalNode *)right)->lr).vect.size(); i++)
        {
          ...
          (((LogicalRelationalNode *)right)->lr).vect.at(i).propagateList.push_back(make_pair(make_pair(this, -0.1), RIGHT));
        }
      }
    }
  }
