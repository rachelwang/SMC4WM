...
#pragma omp parallel num_threads(maxthreads) shared(result, alldone)
    {
        ...
        std::string callTC = TCpath + string(argv[2]) + " " + string(argv[3]) + " "; // only model and property are inputs for TC
        
        while (! alldone) {

            // call TC
            FILE *fp;
            fp = popen(callTC.c_str(), "r");
